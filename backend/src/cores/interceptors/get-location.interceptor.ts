...
import { FIND_NEAREST_STORE, GET_LOCATION } from "../decorators/location.decorator";
import { appConfig } from "src/configs/app.config";
import { StoresService } from "src/apis/stores/stores.service";

@Injectable()
export class GetLocationInterceptor implements NestInterceptor {
    constructor(
    ...
    async intercept(context: ExecutionContext, next: CallHandler<any>): Promise<Observable<any>> {
        const isNearestStoreRequired = this.reflector.get<boolean>(GET_LOCATION, context.getHandler());
        if (!isNearestStoreRequired) return next.handle();

        const request = context.switchToHttp().getRequest();

        const addressShipping = request.body.address;
